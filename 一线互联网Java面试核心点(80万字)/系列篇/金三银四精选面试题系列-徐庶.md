...
![未命名文件.png](./img/FcFu1lLTnLQ5kMP0/1694954914528-22cd7887-cfef-4b9f-afe1-6407adc46c48-738364.png)
...
![image.png](./img/FcFu1lLTnLQ5kMP0/1695020368820-2e3a1c06-f0a4-4421-add3-b773bce34b11-078265.png)

2. ThreadLocal底层是通过ThreadLocalMap来实现的，每个Thread对象（注意不是ThreadLocal对象）中都存在一个ThreadLocalMap，Map的key为ThreadLocal对象，Map的value为需要缓存的值![image.png](./img/FcFu1lLTnLQ5kMP0/1675774309038-18bb28d6-1f45-434b-a182-feb92c7b2933-298813.png)
...
![1684834628234-a5470f8c-2d3e-473f-a2de-e98feba7cc33.png](./img/FcFu1lLTnLQ5kMP0/1684834628234-a5470f8c-2d3e-473f-a2de-e98feba7cc33-650670.png)
```java
...
**公平锁的底层执行流程：**
![ReentrantLock公平锁加锁.png](./img/FcFu1lLTnLQ5kMP0/1626185425264-6d9a8ab7-12d9-4032-8fa4-1bcc40231009-316302.png)
**非公平锁的底层执行流程：**
![ReentrantLock非公平锁加锁.png](./img/FcFu1lLTnLQ5kMP0/1626185427450-d6a6ab8e-94a4-4e7a-be68-bab2d06b320c-272474.png)
                                                   
...
![image.png](./img/FcFu1lLTnLQ5kMP0/1695025035968-02194328-539a-49f8-9097-77ad37b99ba0-228313.png)
...
索引情况：
![image.png](./img/FcFu1lLTnLQ5kMP0/1676441269905-beaa3167-d54b-459f-81b2-8511fa98d205-515128.png)
a字段是主键，对应主键索引，bcd三个字段组成一个联合索引，e字段一个索引

### 1. 不符合最左匹配原则
![image.png](./img/FcFu1lLTnLQ5kMP0/1676440676093-add4ff6a-d422-4f40-b3cb-b37e47649586-941493.png)

去掉b=1的条件就不符合最左匹配原则了，导致所有失效
![image.png](./img/FcFu1lLTnLQ5kMP0/1676440694138-21cb9f9e-62fd-4407-8a59-35dac67b4bb0-555108.png)

### 2. 不正确的Like查询
不用like能走索引：
![image.png](./img/FcFu1lLTnLQ5kMP0/1676441388985-dd894fe7-5e62-4963-82d0-f35527244b5a-620919.png)
正常使用like：
![image.png](./img/FcFu1lLTnLQ5kMP0/1676441439769-58372ca6-248e-4751-8d33-516c484eb3e4-638351.png)
不正确使用like：
![image.png](./img/FcFu1lLTnLQ5kMP0/1676441454918-10d3d846-cb50-49a6-b27d-7d581e47cf6e-466861.png)


### 3. 对索引列进行了计算或使用了函数
![image.png](./img/FcFu1lLTnLQ5kMP0/1676441511562-24c5629c-7592-46fc-ada6-8cb799f74565-584872.png)

![image.png](./img/FcFu1lLTnLQ5kMP0/1676441536615-677899cc-8abb-481a-85b6-61b58ea76179-993758.png)

### 4. 索引列进行了类型转换

e字段的类型是vachar，下面这个sql需要把e字段中的字符转换成数字，会导致索引失效
![image.png](./img/FcFu1lLTnLQ5kMP0/1676441574914-0b982490-b3cb-45ab-a5f5-9b92380e78dc-770343.png)

### 5. <>不等于导致索引失效

b=1可以走索引，b<>1就不能走索引
![image.png](./img/FcFu1lLTnLQ5kMP0/1676441625277-08d21740-9d77-485f-9915-3f43e703cfe2-105992.png)

### 6. order by导致索引失效

就算利用索引，但是由于是select * 所以需要回表，而且回表成本比较高，所以不会走索引。
![image.png](./img/FcFu1lLTnLQ5kMP0/1676442107674-6566e5ed-784a-4325-9869-a9ad0b8bb6e6-045568.png)

如果是select b就需要回表了，就会选择走索引
![image.png](./img/FcFu1lLTnLQ5kMP0/1676442170113-f7ec872a-ddb1-4183-b477-131b056fb588-374782.png)

### 7. 使用or导致索引失效

![image.png](./img/FcFu1lLTnLQ5kMP0/1676442219810-663f0bb0-5ed1-4cac-b93d-be3eaf7452e2-248194.png)

### 8. select * 导致索引失效
![image.png](./img/FcFu1lLTnLQ5kMP0/1676442285345-6dce4534-c3dc-4348-b712-c1ec27164afb-163116.png)
...
![image.png](./img/FcFu1lLTnLQ5kMP0/1676443326948-a9f418ab-1606-47ab-9ebd-618fde89d761-558089.png)
...
![B+树](./img/FcFu1lLTnLQ5kMP0/1686137513424-bd04201b-2fbb-46e8-971b-612c8cef357b-380761.webp)     ![B树](./img/FcFu1lLTnLQ5kMP0/1685543860080-820cb15f-005e-4c95-9342-1b9ee212cc06-381384.png)
